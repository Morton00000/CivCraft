...
import civcraft.slots.machines.SlotCrusher2;
import civcraft.tiles.machines.TileCrusher2;
import cpw.mods.fml.relauncher.Side;
import cpw.mods.fml.relauncher.SideOnly;

public class ContainerCrusher2 extends Container {
	private TileCrusher2 furnace;
	private int lastCookTime = 0;
	private int lastBurnTime = 0;
	private int lastItemBurnTime = 0;

	public ContainerCrusher2(InventoryPlayer par1InventoryPlayer, TileCrusher2 par2TileEntityFurnace) {
		...
		this.addSlotToContainer(new SlotCrusher2(par1InventoryPlayer.player, par2TileEntityFurnace, 2, 116, 35));
		int i;
	...
	public ItemStack transferStackInSlot(EntityPlayer par1EntityPlayer, int par2) {
		...
		if (slot != null && slot.getHasStack()) {
			...
			if (par2 == 2) {
				...
				if (RecipesCrusher2.smelting().getSmeltingResult(itemstack1) != null) {
					if (!this.mergeItemStack(itemstack1, 0, 1, false))
						return null;
				} else if (TileCrusher2.isItemFuel(itemstack1)) {
