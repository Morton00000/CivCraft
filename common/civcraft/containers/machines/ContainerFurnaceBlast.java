...
import civcraft.recipes.machines.RecipesFurnaceBlast;
import civcraft.slots.machines.SlotFurnaceBlast;
import civcraft.tiles.machines.TileFurnaceBlast;
import cpw.mods.fml.relauncher.Side;
import cpw.mods.fml.relauncher.SideOnly;

public class ContainerFurnaceBlast extends Container {
	private TileFurnaceBlast furnace;
	private int lastCookTime = 0;
	private int lastBurnTime = 0;
	private int lastItemBurnTime = 0;

	public ContainerFurnaceBlast(InventoryPlayer par1InventoryPlayer, TileFurnaceBlast par2TileEntityFurnace) {
		...
		this.addSlotToContainer(new SlotFurnaceBlast(par1InventoryPlayer.player, par2TileEntityFurnace, 2, 116, 35));
		int i;
	...
	public ItemStack transferStackInSlot(EntityPlayer par1EntityPlayer, int par2) {
		...
		if (slot != null && slot.getHasStack()) {
			...
			if (par2 == 2) {
				...
				if (RecipesFurnaceBlast.smelting().getSmeltingResult(itemstack1) != null) {
					if (!this.mergeItemStack(itemstack1, 0, 1, false))
						return null;
				} else if (TileFurnaceBlast.isItemFuel(itemstack1)) {
