...
import civcraft.slots.machines.SlotPlasticOven;
import civcraft.tiles.machines.TilePlasticOven;
import cpw.mods.fml.relauncher.Side;
import cpw.mods.fml.relauncher.SideOnly;

public class ContainerPlasticOven extends Container {
	private TilePlasticOven furnace;
	private int lastCookTime = 0;
	private int lastBurnTime = 0;
	private int lastItemBurnTime = 0;

	public ContainerPlasticOven(InventoryPlayer par1InventoryPlayer, TilePlasticOven par2TileEntityFurnace) {
		...
		this.addSlotToContainer(new SlotPlasticOven(par1InventoryPlayer.player, par2TileEntityFurnace, 2, 116, 35));
		int i;
	...
	public ItemStack transferStackInSlot(EntityPlayer par1EntityPlayer, int par2) {
		...
		if (slot != null && slot.getHasStack()) {
			...
			if (par2 == 2) {
				...
				if (RecipesPlasticOven.smelting().getSmeltingResult(itemstack1) != null) {
					if (!this.mergeItemStack(itemstack1, 0, 1, false))
						return null;
				} else if (TilePlasticOven.isItemFuel(itemstack1)) {
