...
import civcraft.recipes.machines.RecipesFurnaceMold;
import civcraft.slots.machines.SlotFurnaceMold;
import civcraft.tiles.machines.TileFurnaceMold;
import cpw.mods.fml.relauncher.Side;
import cpw.mods.fml.relauncher.SideOnly;

public class ContainerFurnaceMold extends Container {
	private TileFurnaceMold furnace;
	private int lastCookTime = 0;
	private int lastBurnTime = 0;
	private int lastItemBurnTime = 0;

	public ContainerFurnaceMold(InventoryPlayer par1InventoryPlayer, TileFurnaceMold par2TileEntityFurnace) {
		...
		this.addSlotToContainer(new SlotFurnaceMold(par1InventoryPlayer.player, par2TileEntityFurnace, 2, 116, 35));
		int i;
	...
	public ItemStack transferStackInSlot(EntityPlayer par1EntityPlayer, int par2) {
		...
		if (slot != null && slot.getHasStack()) {
			...
			if (par2 == 2) {
				...
				if (RecipesFurnaceMold.smelting().getSmeltingResult(itemstack1) != null) {
					if (!this.mergeItemStack(itemstack1, 0, 1, false))
						return null;
				} else if (TileFurnaceMold.isItemFuel(itemstack1)) {
